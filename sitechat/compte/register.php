...
<body>
    ...
    if (isset($_POST) && !empty($_POST)) { /* !empty($_POST) = count($_POST) !== 0 */
        ...
        if (empty($select)) {
            ...
            $msg = "Lien pour vérifier votre adresse mail : http://localhost/exercices/sitechat/compte/verify.php?token=$token";
            SendEmail($_POST['email'], $msg, "Validation Adresse Mail", 'DWWM');

            header("Location: login.php?username=" . $_POST['username']);
        } else 
            echo '<script> alert("Ce pseudo ou l\'addresse email sont déja utilisé donc vous devez en utiliser un autre qui ne soit pas le même mais qui ne comporte pas des caractères spéciaux") </script>';
    }
    ?>
